...
func runConfigImport(cmd *cobra.Command, args []string) error {
	...
	var mode configmanager.ImportMode
	if importFlags.validateOnly {
		mode = configmanager.ModeValidate
	} else if importFlags.overwrite {
		mode = configmanager.ModeOverwrite
	} else if importFlags.dryRun {
		mode = configmanager.ModeValidate // Use validate mode for dry-run
	} else {
...
// MockConfigManager implements ConfigManager for CLI testing
type MockConfigManager struct{}

func (m *MockConfigManager) Export(ctx context.Context, opts configmanager.ExportOptions) (*configmanager.ConfigBundle, error) {
	return &configmanager.ConfigBundle{
		Metadata: configmanager.ConfigMetadata{
			Version:     "1.0.0",
...
func (m *MockConfigManager) ExportToWriter(ctx context.Context, opts configmanager.ExportOptions, w io.Writer) error {
	return nil
}

func (m *MockConfigManager) Import(ctx context.Context, data []byte, opts configmanager.ImportOptions) (*configmanager.ImportResult, error) {
	return &configmanager.ImportResult{
		Success:  true,
		Imported: 1,
		Summary:  configmanager.ImportSummary{},
	}, nil
}

func (m *MockConfigManager) ImportFromReader(ctx context.Context, r io.Reader, opts configmanager.ImportOptions) (*configmanager.ImportResult, error) {
	return m.Import(ctx, []byte{}, opts)
}

func (m *MockConfigManager) Validate(ctx context.Context, data []byte, format configmanager.ConfigFormat) ([]configmanager.ImportError, error) {
	return []configmanager.ImportError{}, nil
}

func (m *MockConfigManager) GetSupportedFormats() []configmanager.ConfigFormat {
	return []configmanager.ConfigFormat{
		configmanager.FormatJSON,
		configmanager.FormatYAML,
		configmanager.FormatHCL,
	}
}

func (m *MockConfigManager) GetVersion() string {
	return "1.0.0"
}
